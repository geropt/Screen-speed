...
/** State carried by an IGNITION_UPDATE event. */
typedef struct {
    bool ignition_on;
} ignition_update_t;
...
typedef struct {
    uint8_t record[RUPTELA_IO_MAX_RECORD_SIZE];
    uint8_t record_len;
} ruptela_record_update_t;
...
typedef enum {
    ...
    IGNITION_UPDATE, /*!< Ruptela IO 409 custom ignition state received */
    GPRS_UPDATE,
    IMEI_UPDATE,
    RECORD_UPDATE
} nmea_event_id_t;
 ...
 *
 * Flushes the UART ring buffer; the event loop and registered handlers
 * are left untouched.
 *
 * @param nmea_hdl handle of NMEA parser
