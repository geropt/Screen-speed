...
typedef struct {
    ...
    ruptela_io_parser_t ruptela_io;                /*!< Binary RS232 I/O record scanner */
    uint8_t imei_overlap[24];                      /*!< Tail used so ###IMEI can straddle UART chunks */
    size_t imei_overlap_len;
    uint64_t last_imei;
    volatile bool ignition_known;                  /*!< At least one valid IO 409 value was received */
    volatile bool ignition_on;                     /*!< Last valid IO 409 value (retained when stale) */
} esp_gps_t;

static esp_err_t post_ignition_update(esp_gps_t *esp_gps, bool ignition_on)
{
    const ignition_update_t update = {
        .ignition_on = ignition_on,
    };

    return esp_event_post_to(esp_gps->event_loop_hdl, ESP_NMEA_EVENT,
                             IGNITION_UPDATE, &update, sizeof(update),
                             pdMS_TO_TICKS(100));
}

static void ruptela_ignition_update(bool ignition_on, void *user_ctx)
{
    esp_gps_t *esp_gps = (esp_gps_t *)user_ctx;
    esp_gps->ignition_on = ignition_on;
    esp_gps->ignition_known = true;

    esp_err_t err = post_ignition_update(esp_gps, ignition_on);
    if (err != ESP_OK) {
        ESP_LOGW(GPS_TAG, "could not post ignition update: %s", esp_err_to_name(err));
    }
}

static void ruptela_gprs_update(bool gprs_up, void *user_ctx)
{
    esp_gps_t *esp_gps = (esp_gps_t *)user_ctx;
    const gprs_update_t update = { .gprs_up = gprs_up };
    esp_err_t err = esp_event_post_to(esp_gps->event_loop_hdl, ESP_NMEA_EVENT,
                                      GPRS_UPDATE, &update, sizeof(update),
                                      pdMS_TO_TICKS(100));
    if (err != ESP_OK) {
        ESP_LOGW(GPS_TAG, "could not post GPRS update: %s", esp_err_to_name(err));
    }
}

static void ruptela_record_update(const uint8_t *record, size_t record_len, void *user_ctx)
{
    esp_gps_t *esp_gps = (esp_gps_t *)user_ctx;
    if (record_len == 0U || record_len > RUPTELA_IO_MAX_RECORD_SIZE) {
        return;
    }
    ruptela_record_update_t update = {0};
    memcpy(update.record, record, record_len);
    update.record_len = (uint8_t)record_len;
    esp_err_t err = esp_event_post_to(esp_gps->event_loop_hdl, ESP_NMEA_EVENT,
                                      RECORD_UPDATE, &update, sizeof(update),
                                      pdMS_TO_TICKS(50));
    if (err != ESP_OK) {
        ESP_LOGW(GPS_TAG, "could not post IO record: %s", esp_err_to_name(err));
    } else {
        ESP_LOGI(GPS_TAG, "IO record len=%u", (unsigned)record_len);
    }
}

static bool parse_imei_marker(const uint8_t *data, size_t len, uint64_t *imei)
{
    static const char prefix[] = "###IMEI";
    const size_t prefix_len = sizeof(prefix) - 1U;
    if (data == NULL || imei == NULL || len < prefix_len + 14U) {
        return false;
    }
    size_t start = 0;
    for (; start + prefix_len <= len; ++start) {
        if (memcmp(data + start, prefix, prefix_len) == 0) {
            break;
        }
    }
    ...
    const uint8_t *digits = data + start + prefix_len;
    const size_t remaining = len - (start + prefix_len);
    uint64_t value = 0;
    size_t count = 0;
    while (count < remaining && digits[count] >= '0' && digits[count] <= '9') {
        value = value * 10ULL + (uint64_t)(digits[count] - '0');
        ++count;
    }
    if (count < 14U || count > 16U) {
        return false;
    }
    *imei = value;
    return true;
...
#define IMEI_OVERLAP 24

static void consider_imei(esp_gps_t *esp_gps, const uint8_t *data, size_t len)
{
    uint8_t tmp[IMEI_OVERLAP + NMEA_PARSER_RUNTIME_BUFFER_SIZE];
    size_t n = 0;
    if (esp_gps->imei_overlap_len > 0 && esp_gps->imei_overlap_len <= IMEI_OVERLAP) {
        memcpy(tmp, esp_gps->imei_overlap, esp_gps->imei_overlap_len);
        n = esp_gps->imei_overlap_len;
    }
    if (len > NMEA_PARSER_RUNTIME_BUFFER_SIZE) {
        len = NMEA_PARSER_RUNTIME_BUFFER_SIZE;
    }
    memcpy(tmp + n, data, len);
    n += len;
    uint64_t imei = 0;
    if (parse_imei_marker(tmp, n, &imei) && imei != esp_gps->last_imei) {
        esp_gps->last_imei = imei;
        const imei_update_t update = { .imei = imei };
        esp_err_t err = esp_event_post_to(esp_gps->event_loop_hdl, ESP_NMEA_EVENT,
                                          IMEI_UPDATE, &update, sizeof(update),
                                          pdMS_TO_TICKS(50));
        if (err != ESP_OK) {
            ESP_LOGW(GPS_TAG, "could not post IMEI: %s", esp_err_to_name(err));
        }
    }
    if (n > IMEI_OVERLAP) {
        memcpy(esp_gps->imei_overlap, tmp + n - IMEI_OVERLAP, IMEI_OVERLAP);
        esp_gps->imei_overlap_len = IMEI_OVERLAP;
    } else {
        memcpy(esp_gps->imei_overlap, tmp, n);
        esp_gps->imei_overlap_len = n;
    }
}

/** Pull every pending UART byte. Do not wait for '\\n': IO records are binary
 *  and a 0x0A in the payload used to fire pattern-detect and flush the FIFO. */
static void drain_uart(esp_gps_t *esp_gps)
{
    while (1) {
        size_t avail = 0;
        if (uart_get_buffered_data_len(esp_gps->uart_port, &avail) != ESP_OK || avail == 0) {
            break;
        }
        if (avail > NMEA_PARSER_RUNTIME_BUFFER_SIZE - 1) {
            avail = NMEA_PARSER_RUNTIME_BUFFER_SIZE - 1;
        }
        int n = uart_read_bytes(esp_gps->uart_port, esp_gps->buffer, avail, 0);
        if (n <= 0) {
            break;
        }
        ruptela_io_parser_feed(&esp_gps->ruptela_io, esp_gps->buffer, (size_t)n);
        consider_imei(esp_gps, esp_gps->buffer, (size_t)n);
        esp_gps->buffer[n] = '\0';
        if (gps_decode(esp_gps, (size_t)n + 1U) != ESP_OK) {
...
nmea_parser_handle_t nmea_parser_init(const nmea_parser_config_t *config)
{
    ...
    ruptela_io_parser_init(&esp_gps->ruptela_io, ruptela_ignition_update, esp_gps);
    ruptela_io_parser_set_gprs_callback(&esp_gps->ruptela_io, ruptela_gprs_update);
    ruptela_io_parser_set_record_callback(&esp_gps->ruptela_io, ruptela_record_update);
...
{
    esp_gps_t *esp_gps = (esp_gps_t *)nmea_hdl;
    esp_err_t err = esp_event_handler_register_with(esp_gps->event_loop_hdl, ESP_NMEA_EVENT,
                                                    ESP_EVENT_ANY_ID, event_handler,
                                                    handler_args);
    if (err != ESP_OK || !esp_gps->ignition_known) {
        return err;
    }

    /* The parser task starts during nmea_parser_init(). Replay a value that
     * arrived in the short interval before this handler was registered. A
     * concurrent duplicate is harmless because the display API is idempotent. */
    return post_ignition_update(esp_gps, esp_gps->ignition_on);
}
...
esp_err_t nmea_parser_set_baud(nmea_parser_handle_t nmea_hdl, uint32_t baud_rate)
{
    ...
    esp_gps->crc = 0;
    esp_gps->parsed_statement = 0;
    esp_gps->imei_overlap_len = 0;
    ruptela_io_parser_reset(&esp_gps->ruptela_io);
    return ESP_OK;
}
